...
class MLPPool(MLP):
    def __init__(
        ...
        super(MLPPool, self).__init__(input_size+pool_size, hidden_size, output_size, dropout, num_hidden, model_name)
...
class MultiMLPSAGE(GeneralMultiMLPModel):
    def __init__(
        self,
        run_config,
        input_size,
        ...
        super(MultiMLPSAGE, self).__init__(run_config, input_size, output_size, device, rng, eps, num_hidden, model_name)
...
if __name__ == "__main__":
    ...
    model = MultiMLPSAGE(RunConfig(), 10, 2, torch.rand(10, 10), "cpu", np.random)
    print(model)
