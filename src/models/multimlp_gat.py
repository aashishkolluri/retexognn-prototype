...
class GATPool(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, dropout, alpha, num_heads):
        super(GATPool, self).__init__()
        
        self.dropout = dropout
        self.model_name = "gatpool"
        
        self.attentions = [GraphAttentionLayer(input_size, hidden_size, dropout=dropout, alpha=alpha, concat=True) for _ in range(num_heads)]
        
        for i, attention in enumerate(self.attentions):
            self.add_module('attention_{}'.format(i), attention)
 
        self.linear_layer = nn.Linear(hidden_size * num_heads, output_size, bias=False)
...
class MultiMLPGAT(GeneralMultiMLPModel):
    def __init__(
        self,
        run_config,
        input_size,
        ...
        super(MultiMLPGAT, self).__init__(run_config, input_size, output_size, device, rng, eps, num_hidden, model_name)
        ...
        for _ in range(1, self.nl):
            self.model_list.append(GATPool(
                input_size=input_size,
...
if __name__ == "__main__":
    ...
    model = MultiMLPGAT(RunConfig(), 10, 2, torch.rand(10, 10), "cpu", np.random)
    print(model)
